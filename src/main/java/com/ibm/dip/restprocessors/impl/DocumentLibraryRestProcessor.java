...
@Service
public class DocumentLibraryRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(DocumentLibraryRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.documentlibrary.hosturl}")
	private String url;
	
	@Value("${spring.api.documentlibrary.context-path}")
	private String contextPath;

	@Value("${spring.api.documentlibrary.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  DocumentLibraryRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ DocumentLibraryRestProcessor.class.getName());
